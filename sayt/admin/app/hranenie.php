...
function sdelat_kopiyu(): ?string
{
    ...
    // Метка времени с точностью до секунды, а две правки подряд укладываются
    // в одну: вторая копия затирала первую — ту самую, к которой и хочется
    // вернуться, заметив опечатку сразу после сохранения. Поэтому занятое
    // имя дополняется номером.
    $osnova = PAPKA_KOPIJ . '/menu-' . date('Y-m-d_H-i-s');
    $imya = $osnova . '.json';
    // Номер отделяется подчёркиванием, а не дефисом: старые копии удаляются
    // по строковой сортировке имён, и дефис встал бы перед точкой основного
    // имени — свежая копия оказалась бы «старше» и ушла первой.
    for ($n = 2; is_file($imya) && $n < 100; $n++) {
        $imya = $osnova . '_' . sprintf('%02d', $n) . '.json';
    }
